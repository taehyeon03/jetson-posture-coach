...
세부 측정 범위·합격 기준·비교 실험·Git 결과 형식은 [시스템 검증 계획](docs/system_validation_plan.md)에 정리합니다.

측정 도구는 `scripts/benchmark.py`이며 사용법과 지표 해석은 [벤치마크 안내](benchmarks/README.md)에 있습니다(지연·처리량·안정성·인식율·에너지·메모리·왕복 지연·추론 비용).
아직 하지 않은 실측과 확장 항목은 [TODO](TODO.md)에 모았습니다.
